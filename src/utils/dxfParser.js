...
export const parseDXF = async (file, husoSelection) => {
  ...
  // DXF has no strict EPSG inside it normally in this context, we require the user's selected HUSO
  const epsgCode = husoSelection || "25830";
  ...
  // Ray-casting point in polygon algorithm exactly as provided
  ...
  // 1. Recoger todos los textos
  dxf.entities.forEach(entity => {
      if (entity.type === "TEXT" || entity.type === "MTEXT") {
          if (!entity.text) return;

          // Text position can be in position, startPoint, or flat x/y depending on DXF version
  ...
  dxf.entities.forEach(entity => {
    ...
    // Force geometrically closing any polyline since many CAD workflows
    // leave adjacent interior geometries structurally unclosed.
    const first = coords[0];
    const last = coords[coords.length - 1];

    if (Math.abs(first[0] - last[0]) > 0.001 || Math.abs(first[1] - last[1]) > 0.001) {
        coords.push([...first]); // Ensure closing point is added
    }

    let name = null;

    // buscar texto dentro de la parcela
    for (let t of texts) {
    ...
    parcels.push({
        id: `dxf-${file.name}-${parcels.length}`,   // unique: filename + sequential index
        name: name,
        area: area,
        filename: file.name,
        huso: epsgCode,
  ...
  // Filtrar duplicados exactos (mismo área y mismo vértice inicial)
  // Es muy común que los programas CAD exporten geometrías solapadas
  const uniqueParcels = [];
  parcels.forEach(p => {
